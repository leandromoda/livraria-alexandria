...
def _check(conn, descricao, query, acao_recomendada):
    """Executa um check, loga e retorna um dict estruturado do resultado."""
    cur = conn.cursor()
    cur.execute(query)
    n = cur.fetchone()[0]
    if n > 0:
...
def run(idioma: str = None, pacote: int = None):
    """Executa todos os checks de integridade, loga e emite relatório padronizado.

    Saída: scripts/data/logs/NNNN_audit_integrity.json (consumido pelo /audit).
    """
    ...
    log("=" * 52)
    if checks_com_problema == 0:
        log("[AUDIT] ✅  Nenhuma inconsistência encontrada. Pipeline íntegro.")
    else:
        log(f"[AUDIT] ⚠️  {checks_com_problema} checks com problemas — revisar ações recomendadas acima.")
    log("=" * 52)

    # Relatório padronizado para o /audit
    from core.audit_report import save_audit_report
    report = {
        "mode": "integrity",
        "total_checks": len(results),
        "checks_with_issues": checks_com_problema,
        "total_registros_afetados": sum(r["count"] for r in results),
        "results": results,
    }
    path = save_audit_report(report)
    log(f"[AUDIT] Relatório salvo: {path}")
    return report
